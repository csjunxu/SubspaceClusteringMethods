...
%          'warmstart' - add user's 'seeds' into the seeds set from 'sample'
...
[n, p] = size(X);
% 'warmstart' means to re-use the last result and hence need a 'initial_idx': idx0
pnames = { 'distance'  'start' 'replicates' 'emptyaction' 'onlinephase' 'options' 'maxiter' 'display' 'initiallabel'};
dflts =  {'sqeuclidean' 'sample'          []         'error'         'on'        []        []        []    []};
[eid,errmsg,distance,start,reps,emptyact,online,options,maxit,display, idx0] ...
...
if ischar(start)
    % 'warmstart' means to re-use the last result
    startNames = {'uniform','sample','cluster','warmstart'};
...
for rep = 1:reps
    ...
    case 'warmstart'
        % need provide 'idx0' after keyword 'initiallabel'
        if (length(unique(idx0))==k)
            for ii =1: n
                idx_i = (idx0 ==ii);
                C(ii, :) = mean(X(idx_i, :), 1);
            end          
